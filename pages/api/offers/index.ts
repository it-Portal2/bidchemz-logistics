  ...
  if (req.method === "GET") {
    ...
    try {
      ...
      const subscriptionTier =
        partnerCapability?.subscriptionTier || SubscriptionTier.FREE;
      ...
      const result = await prisma
        .$transaction(async (tx) => {
          ...
          if (wallet.balance < leadCost) {
            ...
            throw new Error(
              `Insufficient wallet balance. Required: ₹${leadCost.toFixed(
                2
          ...
          if (updateResult.count === 0) {
            ...
            throw new Error(
              `Insufficient wallet balance. Another transaction may have reduced your balance. Please check and try again.`
            );
          }
